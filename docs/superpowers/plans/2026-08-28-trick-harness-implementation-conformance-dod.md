...
**Architecture:** Extend the generic contracts with approved-artifact identities and structured conformance results; build a deterministic manifest before model dispatch so required obligations cannot be omitted by the model; insert conformance before final verification in the PR lifecycle; route it as independent Codex judgement work according to risk; surface bounded conformance facts through control/journal/composition. NeuroVia project policy supplies approved artifact paths and project DoD inputs later through the Plan C* conformance wiring overlay.
...
- [ ] **Step 1: Write RED contract tests** proving `ROLES` contains `conformance`, `READ_ONLY_ROLES` contains `conformance`, `parseWorkflowObjective` requires `approvedArtifacts`, hashes must match lowercase 64-hex SHA-256, and artifact paths must be non-empty repository-relative strings rather than absolute paths.
- [ ] **Step 2: Run RED.**
...
- [ ] **Step 3: Implement the vocabulary/types/parser** in `types.ts` and `index.ts`. Reject absolute paths, `..` path traversal segments, missing hashes and malformed hashes without echoing offending values into errors.
- [ ] **Step 4: Run GREEN.**
...
- [ ] **Step 1: Write RED parser tests** for valid contract, unknown source/status, duplicate/missing fields, extra transcript/reasoning fields, malformed hashes and secret-safe errors.
- [ ] **Step 2: Implement `parseConformanceContract(value, path='conformance')`** using the existing bounded parser helpers and immutable outputs.
...
- Modify: `packages/core/engineering-workflow/src/index.ts` only to export the new public helpers if needed by the host
...
DoD obligations:
  supplied by deterministic profile/deployment policy; duplicate ids rejected
```
...
- [ ] **Step 2: Write RED lifecycle test** expecting stage order `implement -> verify -> delivery -> review -> applicable qa -> applicable security -> conformance -> verify-final`.
...
- [ ] **Step 4: Implement workflow integration.** The conformance interpreter output is parsed and coverage-validated before it becomes stage facts. A missing `request.conformance` for a PR lifecycle is `INCONCLUSIVE`, not implicit PASS.
...
export const pluroraDodObligations: readonly ConformanceObligation[] = [
  { id: 'DOD-APPROVED-ARTIFACTS', source: 'dod', requirement: '...', required: true },
  { id: 'DOD-DIFF-COHERENCE', source: 'dod', requirement: '...', required: true },
  { id: 'DOD-FRESH-EVIDENCE', source: 'dod', requirement: '...', required: true },
  { id: 'DOD-NO-MATERIAL-DEFECT', source: 'dod', requirement: '...', required: true },
  { id: 'DOD-APPLICABLE-QA', source: 'dod', requirement: '...', required: true },
  { id: 'DOD-APPLICABLE-SECURITY', source: 'dod', requirement: '...', required: true },
  { id: 'DOD-DELIVERY-WORLD', source: 'dod', requirement: '...', required: true },
  { id: 'DOD-FINAL-VERIFICATION', source: 'dod', requirement: 'prerequisite state is ready for final verification', required: true },
]
```
...
- [ ] **Step 1: Write RED tests** for all four risk levels and the Codex-unavailable fallback.
- [ ] **Step 2: Add RED independence tests** proving high/critical conformance cannot certify when implementation executor is OpenCode and fallback also resolves to OpenCode under `cross-executor-required`.
- [ ] **Step 3: Add the baseline DoD policy** and profile composition reference without embedding NeuroVia file paths, database refs or native model ids.
- [ ] **Step 4: Implement routing rows before generic review/default rows** so specific conformance routes win deterministically.
- [ ] **Step 5: Run GREEN.**
...
- Modify: `packages/core/journal/src/projection.ts` if projection is split there in the implementation branch
- Test: `packages/core/journal/tests/journal.spec.ts`
- Test: `packages/core/journal/tests/restart.spec.ts` if the current branch keeps restart tests separately
...
- [ ] **Step 1: Write RED API/parser tests** proving workflow start accepts the approved artifact set, rejects malformed/absolute/traversal paths, and status exposes only bounded hashes/counts/verdict rather than whole document/model output.
- [ ] **Step 2: Write RED journal tests** proving artifact identity and conformance summary survive replay/restart and no event key stores `specText`, `planText`, `prompt`, `transcript`, `reasoning` or model output.
- [ ] **Step 3: Implement control and durable projection changes** using existing parser/journal conventions.
- [ ] **Step 4: Run GREEN.**
...
**Files:**
- Modify after Plan E creates them: `apps/plurora-harness-host/src/main.ts`
- Modify after Plan E creates them: `apps/plurora-harness-host/src/model-registry.ts`
- Modify after Plan E creates them: `apps/plurora-harness-host/src/workflow-handlers.ts`
- Modify after Plan E creates them: `apps/plurora-harness-host/tests/model-registry.spec.ts`
- Modify after Plan E creates them: `apps/plurora-harness-host/tests/host.spec.ts`
- Modify only if Plan E has not already exposed the native catalogue field: `packages/subagent/subagent-codex/src/wire.ts`
- Test when wire changes: `packages/subagent/subagent-codex/tests/subagent-codex.spec.ts`

- [ ] **Step 1: Write RED host tests** proving conformance receives current approved artifact texts only after host-side path containment/hash verification and receives the deterministic manifest including Plurora DoD rows.
...
- [ ] **Step 3: Use the pinned app-server `model/list` response** already introduced/validated by Plan E. Preserve advertised effort strings/order; do not maintain a guessed global ranking table in project code.
...
corepack pnpm vitest run packages/subagent/subagent-codex/tests/subagent-codex.spec.ts profiles/plurora/tests/routing.spec.ts
```
...
- Modify: `docs/verification/2026-08-27-neurovia-deployment-enablement-evidence.md` from Plan E only to mark its SHA as intermediate when applicable
...
- [ ] **Step 2: Run a fixture PR lifecycle whose Spec has two criterion IDs and Plan has two tasks; prove the manifest contains exactly 2 Spec + 2 Plan + 8 baseline DoD rows and `PR_READY` is reached only after conformance PASS + verify-final PASS.**
- [ ] **Step 3: Run adversarial fixtures:** omitted Plan task, changed Spec hash, duplicate result item, unknown obligation substitution and high-risk OpenCode implementation with Codex unavailable. Each must fail/block/inconclusive according to the Spec rather than reaching `PR_READY`.
- [ ] **Step 4: Run real authenticated Codex catalogue evidence** showing the concrete models mapped to `codex.balanced` and `codex.frontier` advertise `high` and `xhigh` respectively; record ids but no credentials.
...
- [ ] **Step 6: Record the final reviewed exact 40-hex SHA.** This post-Plan-F SHA supersedes any intermediate Plan E SHA as the only initial runtime revision Plan C* may pin.
