...
## Scoped one-shot transport

`startCodexTask(request, spec)` is the same lifecycle without the shared subagent request shape: it takes the task text and a cancellation signal, and it accepts an optional `routing` object carrying `model` and `effort` for that one run. `startCodexRun` is now a thin adapter over it that routes nothing, so a run started through the Provider emits exactly the frames it emitted before this seam existed.

Both field names come from the app-server JSON schema generated by the pinned package, not from documentation. `TurnStartParams.model` is `string | null` and `TurnStartParams.effort` is `ReasoningEffort | null`; `ThreadStartParams` has no effort field at all, which is why both overrides travel on `turn/start` rather than on the thread. `ReasoningEffort` is a non-empty string rather than a closed enum, because the accepted set is advertised per model through `Model.supportedReasoningEfforts`, so validation here checks only what the schema constrains.

An absent field is omitted from the frame rather than sent as an explicit null, and a supplied field that is not a non-empty value is rejected before a process is spawned. Routing never reads or writes `CODEX_HOME`, a Codex profile, or any other global product default.
