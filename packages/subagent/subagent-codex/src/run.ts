...
/**
 * Per-run routing overrides carried on `turn/start`.
 *
 ...
 * documentation: `TurnStartParams.model` is `string | null`, and
 * `TurnStartParams.effort` is `ReasoningEffort | null`. `effort` exists only on
 * `turn/start` — `ThreadStartParams` has no effort field at all — which is why
 * both overrides travel on the turn rather than the thread.
 *
