...
External certification produced fresh evidence of its own: deterministic gates, a fourteen-row adversarial matrix driven through the real Plurora profile over a scripted subprocess seam, and a real authenticated GitHub canary on the Plan H pull request in a dedicated non-required context and a non-success state, independently re-read through the API. The harness now publishes one commit status on the re-read head of the pull request it delivered: `pending` before any certifying stage continues, and a terminal state afterwards that reaches `success` only from the exact `PR_READY` decision for that same head. Descriptions are fixed strings selected from the state alone and the target URL is the verified pull-request URL, so no prompt, model output, path, database URL or credential can reach a status field. The capability has no commit, push, pull-request-edit, merge, release or deploy method, and native `gh` authentication remains the only credential path. What has not been proven is stated there too: the production context `plurora/harness-certification` has never been published to against a real repository, no `success` has been published to real GitHub, and no branch-protection rule yet requires the context. See `docs/verification/2026-08-28-harness-v2-plan-h-certification-evidence.md`, which supersedes the implementation-conformance evidence as the installation authority; the SHA recorded there is the only initial runtime revision a deployment may pin.
