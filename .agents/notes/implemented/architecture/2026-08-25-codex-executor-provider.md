...
The routable set is enforced at runtime, not by the type alone. `CodexRoutedSandbox` keeps `danger-full-access` off a route for callers the compiler saw, but the transport is exported and is therefore a boundary that parsed configuration, JavaScript, and an assertion all cross; a privilege ceiling that only holds for type-checked callers is not a ceiling. `assertRouting` checks `sandbox` against `CODEX_ROUTED_SANDBOXES` before anything is spawned.

One consequence is worth stating rather than leaving to be discovered. Against the bypass mode a routed sandbox only ever narrows, and against `approve-for-me` it can narrow to `read-only`. Against `never` — the default — the permission mode emits no sandbox at all and defers to the user's own Codex configuration, so a routed `workspace-write` grants more authority than that configuration would have. That is the intended contract, because the route is the authority decision for the run, but it is a grant and not merely a restriction, and it is written down at the emission site as one.
